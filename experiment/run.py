    ...
    for run in range(params.num_runs):
        ...
        if params.online:
            for i, (x_train, y_train, labels) in enumerate(data_continuum):
                ...
                if params.save_cp or params.hardness_analysis:
                    agent.train_learner(x_train, y_train, run, i)
                else:
                    agent.train_learner(x_train, y_train, run, i)           
                # if hasattr(agent, 'end_task'):
                #     agent.end_task(x_train, y_train)
                acc_array = agent.evaluate(test_loaders)
                tmp_acc.append(acc_array)
                # if hasattr(agent, 'context'):
                #     context_multiplication(agent.model, agent.context, agent.task)
                ...
                if hasattr(agent, 'task'):
                    agent.task += 1
                # if params.save_cp:
                #     filename = f"{params.exp}checkpoint_run{run}_batch{i}.pth"
                #     filepath = os.path.join(params.cp_path, filename)
                #     grads = {name: parameter.grad.clone() for name, parameter in agent.named_parameters() if parameter.grad is not None}
                ...
                #         'eps_mem_batch': agent.eps_mem_batch,
                #         'mem_iters': agent.mem_iters,
                #         'triplet': agent.triplet,
                #         'top5': agent.top5,
                #         'buffer2': agent.buffer2.state_dict() if hasattr(agent, 'buffer2') else None,
                #     }
