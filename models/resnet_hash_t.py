...
from models import real_test as real
...
class HashBasicBlock(nn.Module):
    ...
    #     out = F.relu(self.bn1(self.conv1(x, time, z)))
    #     out = self.bn2(self.conv2(out, time, z))
    #     if len(self.shortcut) > 0:
    #         sout = self.shortcut[0](x, time, z)
    #         sout = self.shortcut[1](sout, z)
    #     else:
    #         sout = x
    #     out += sout 
    #     out = F.relu(out)
...
class HashResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10):
        ...
        self.cheat_period = 1000000
        self.time_slow = 20000
        # self.z = nn.Parameter(torch.randn(20))
    ...
    # def freeze_z_up_to_task(self, task_id):
    # # 在 backward 过程中手动创建一个梯度掩码
    #     def backward_hook(grad):
    #         mask = torch.zeros_like(grad)
    #         # 允许 task_id 及之后的 z 元素更新梯度
    #         mask[task_id:] = 1  # 这里是 task_id 而不是 task_id+1
    #         return grad * mask

    #     # 注册一个钩子函数，应用掩码到 z 的梯度上
    #     self.z.register_hook(backward_hook)
