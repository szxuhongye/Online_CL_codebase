...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes, nf, bias, memory_bank_size, num_subcentroids, gamma):
        ...
        # self.pcrLinear = cosLinear(160, 100)
        ...
        self.gamma = gamma
        self.pretrain_subcentroids = True
        ...
        self.prototypes = nn.Parameter(torch.zeros(self.num_classes, self.num_subcentroids, self.embedding_dim))
        init.kaiming_normal_(self.prototypes, mode='fan_out', nonlinearity='relu')
    ...
    def subcentroids_learning(self, _c, out_seg, gt_seg, masks): 
        ...
        for k in range(self.num_classes):
            ...
            # if init_q.shape[0] == 0 or init_q.shape[0] == 1:
            #     continue
            if init_q.shape[0] == 0:
                continue

            # clustering q.shape = n x self.num_subcentroids
            # q, indexs = approx_ot(init_q) ##
            ...
            # print("indexs", indexs)
            # print("gt_seg", gt_seg)
            # print("centroid_target", centroid_target)
            # exit(0)
    ...
    def forward(self, x, gt_label, **kwargs):
        ...
        if self.use_subcentroids is True and self.isOnlyCE is False:
        
            # concat data from memory_bank
            origin_inputs, inputs = inputs, torch.cat(self.memory_bank,0)
            gt_label = torch.cat(self.memory_bank_label,0)

            seg_logits, contrast_logits, contrast_target = self.forward_train(inputs, gt_label=gt_label, origin_x = origin_inputs)
    ...
    def forward_train(self, x, gt_label=None, origin_x=None):
        ...
        if origin_x is not None:
            origin_x = self.feat_norm(origin_x)
            origin_x = self.l2_normalize(origin_x)
        ...
        if origin_x is not None:
            origin_masks = torch.einsum('nd,kmd->nmk', origin_x, self.prototypes)
            origin_out_cls = torch.amax(origin_masks, dim=1)
            origin_out_cls = self.mask_norm(origin_out_cls)
        ...
        if self.use_subcentroids is True and gt_label is not None and self.isOnlyCE is False:
            contrast_logits, contrast_target = self.subcentroids_learning(x, out_cls, gt_label, masks)
            return origin_out_cls, contrast_logits, contrast_target
...
def Reduced_ResNet18_m(nclasses, nf=20, bias=True, memory_bank_size = 1, num_subcentroids=4, gamma=0.9):
    """
    Reduced ResNet18 as in GEM MIR(note that nf=20).
    """
    return ResNet(BasicBlock, [2, 2, 2, 2], nclasses, nf, bias, memory_bank_size, num_subcentroids, gamma)

def Reduced_ResNet34_m(nclasses, nf=20, bias=True, memory_bank_size = 1, num_subcentroids=4, gamma=0.9):
    """
    """
    return ResNet(BasicBlock, [3, 4, 6, 3], nclasses, nf, bias, memory_bank_size, num_subcentroids, gamma)

def Reduced_ResNet50_m(nclasses, nf=20, bias=True, memory_bank_size = 1, num_subcentroids=4, gamma=0.9):
    """
    """
    return ResNet(Bottleneck, [3, 4, 6, 3], nclasses, nf, bias, memory_bank_size, num_subcentroids, gamma)

def Reduced_ResNet101_m(nclasses, nf=20, bias=True, memory_bank_size = 1, num_subcentroids=4, gamma=0.9):
    """
    """
    return ResNet(Bottleneck, [3, 4, 23, 3], nclasses, nf, bias, memory_bank_size, num_subcentroids, gamma)

def Reduced_ResNet152_m(nclasses, nf=20, bias=True, memory_bank_size = 1, num_subcentroids=4, gamma=0.9):
    """
    """
    return ResNet(Bottleneck, [3, 8, 36, 3], nclasses, nf, bias, memory_bank_size, num_subcentroids, gamma)
