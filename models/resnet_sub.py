...
class cosSubCentroids(nn.Module):

    def __init__(self, indim, outdim, num_subcentroids):
        super(cosSubCentroids, self).__init__()
        init_factor = 2.0  # init_factor
        self.scale = 0.09
        # self.prototypes = nn.Parameter(torch.zeros(self.num_classes, self.num_subcentroids, self.embedding_dim))
        self.prototypes = nn.Parameter(torch.zeros(outdim, num_subcentroids, indim))
        init.kaiming_normal_(self.prototypes, a=init_factor, mode='fan_out', nonlinearity='relu')
    ...
    def forward(self, x):
        ...
        prototypes_norm = torch.norm(self.prototypes, p=2, dim =-1).unsqueeze(2).expand_as(self.prototypes)
        weight_normalized = self.prototypes.div(prototypes_norm + 0.000001)
        masks = torch.einsum('nd,kmd->nmk', x, weight_normalized)
        cos_dist = torch.amax(masks, dim=1) 
        scores = cos_dist / self.scale
        return scores

class ResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes, nf, bias, num_subcentroids=5):
        ...
        self.pcrLinear_sub = cosSubCentroids(nf * 8 * block.expansion, num_classes, num_subcentroids)
    ...
    def pcrForward(self, x):
        out = self.features(x)
        logits = self.pcrLinear_sub(out)
        return logits, out
    # MLP class for projector and predictor


def Reduced_ResNet18_sub(nclasses, nf=20, bias=True, num_subcentroids=5):
    """
    Reduced ResNet18 as in GEM MIR(note that nf=20).
    """
    return ResNet(BasicBlock, [2, 2, 2, 2], nclasses, nf, bias, num_subcentroids)
