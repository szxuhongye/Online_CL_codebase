...
class LipOptimizer(ContinualLearner):

    def __init__(self, model, opt, params):
        super().__init__(model, opt, params)
        self.args = params

        self.args.lip_compute_mode = self.args.lip_compute_mode if hasattr(params, "lip_compute_mode") else "different_layer"
        self.args.lip_difference_mode = self.args.lip_difference_mode if hasattr(params, "lip_difference_mode") else "sample"
    ...
    def init_net(self, trainloader):
        ...
        for i, (inputs, labels) in enumerate(tqdm(trainloader, desc="Evaluating initial L")):
            ...
            _, partial_features = self.model(inputs, returnt='full')
        ...
        inp = next(iter(trainloader))[0]
        _, teacher_feats = self.model(inp.to(self.device), returnt='full')

        self.model.lip_coeffs = torch.autograd.Variable(torch.randn(len(teacher_feats)-1, dtype=torch.float), requires_grad=True).to(self.device)
        self.model.lip_coeffs.data = self.budget_lip.detach().clone()
        self.opt = torch.optim.SGD(self.model.parameters(), lr=self.args.lr,
    ...
    def budget_lip_loss(self, features: List[torch.Tensor]) -> torch.Tensor:
        ...
        if self.args.headless_init_act == "relu":
            ...
            tgt = F.leaky_relu(self.model.lip_coeffs[:len(lip_values[0])])
        else:
            assert False
