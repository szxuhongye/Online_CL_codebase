...
class PLACES100(DatasetBase):
    def __init__(self, scenario, params):
        ...
        super(PLACES100, self).__init__(dataset, scenario, num_tasks, params.num_runs, params)


    def download_load(self):
        # plaeces100_root = '/home/hx5239/places365/places365_standard/train'
        self.train_data, self.train_label = self.load_data('train.lst', self.root)
        self.test_data, self.test_label = self.load_data('test.lst', self.root)
    ...
    def load_data(self, filne_name, dataset_path):
        ...
        with open(os.path.join(self.root, filne_name), 'r') as file:
            for line in file:
                ...
                # try:
                new_size = (224, 224)
                image = Image.open(full_path).convert("RGB").resize(new_size)
