                ...
                for j in range(self.mem_iters):
                    ...
                    if self.params.second_buffer:
                        if self.params.buffer_analyze:
                            mem_x1, mem_y1, buffer_x1, buffer_y1 = self.buffer.retrieve(x=batch_x, y=batch_y)
                            mem_x2, mem_y2, buffer_x2, buffer_y2 = self.buffer2.retrieve(x=batch_x, y=batch_y)
                        else:
                        ...
                        if self.params.buffer_analyze:
                            mem_x, mem_y, buffer_x, buffer_y = self.buffer.retrieve(x=batch_x, y=batch_y)
                        else:
            ...
            if self.params.buffer_analyze:
                if (ep + 1) % 10 == 0:
                    ...
                    if self.params.second_buffer: # concat mem_x1 with mem_x2, buffer_x1 with buffer_x2, etc.
                        ...
                        np.savez(filepath, mem_x=mem_x, mem_y=mem_y, buffer_x=buffer_x, buffer_y=buffer_y)
                    else:
